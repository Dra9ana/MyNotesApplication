...
# Defining class for a Passwords section
class Travel:
     def __init__(self, website, additional_info, username, mail, password):
...
# Reading information from the input
    website         = input1.get()
    additional_info = input2.get()
    username        = input3.get()
    mail            = input4.get()
    password        = input5.get()
    
# Opening/Closing a file and updating it
    input_file = open("Passwords_input.txt","w");
    input_file. write(website+" "+additional_info+" "+usernmae+" "+mail+" "+password);
    input_file.close()
...
# Creating graphics
  ...
  # Labelling Important Points
    tk.Label(new_master, text="Website:"+website).grid(row=0)
    tk.Label(new_master, text="Additional info:"+additional_info).grid(row=1)
    tk.Label(new_master, text="Username:"+username).grid(row=2)
    tk.Label(new_master, text="Mail:"+mail).grid(row=3)
    tk.Label(new_master, text="Password:"+password).grid(row=4)
...
tk.Label(master, text="Additional info:").grid(row=1)
tk.Label(master, text="Username:").grid(row=2)
tk.Label(master, text="Mail:").grid(row=3)
tk.Label(master, text="Password:").grid(row=4)
