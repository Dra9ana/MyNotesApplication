...
def new_note (list_box, indexes_list):
    ...
    tk.Button(master, text='Quit', command=master.destroy).grid(row=7, column=1, sticky=tk.W,pady=4)
    tk.Button(master, text='Done', command=lambda:read_input(master, input1, input2, input3, input4, input5, input6, list_box, indexes_list)).grid(row=7, column=2, sticky=tk.W, pady=4)
